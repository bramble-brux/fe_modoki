...
- **Free の延長は「厳禁」**: 休憩の延長は集中サイクルを崩壊させる。目標到達で即座にアラートを鳴らし、アプリ起動中は3秒ごとにアラートを繰り返す。タイマーは 0:00 で停止し、コーラル色で警告表示にする。
...
- **Free の延長は厳禁。** アプリ起動中は3秒ごとにアラート音を鳴らし続け、1分ごとに通知で催促する。
...
- バックグラウンド移行時に `backgroundDate` を記録。Free 超過アラートタイマーは停止。
- フォアグラウンド復帰時に差分を `elapsed` に加算してキャッチアップ。
- **一時停止中はキャッチアップしない。**
- Free 超過状態で復帰した場合、アラートを再開する。
